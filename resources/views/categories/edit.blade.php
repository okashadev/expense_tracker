<x-app-layout>
    <div class="px-4 py-2 w-full">
        <div class="flex w-full justify-between items-center pb-8">
            <h1 class="text-4xl text-[#254D70] font-extrabold">Edit Category</h1>
        </div>
        ...
        <div class="w-full bg-white p-10 rounded-lg shadow-lg">
            <form action="{{ route('categories.update', $category->id) }}" method="POST">
                ...
                <div class="mb-5">
                    ...
                    <input type="text" id="category_name" value="{{ $category->name }}" name="name" required
                        placeholder="Enter Category name"
                        class="bg-gray-50 border capitalize border-gray-300 text-black text-sm rounded-lg focus:ring-[#954C2E] focus:border-[#954C2E] block w-full p-2.5" />
                </div>

                <div class="mb-5" x-data="{ selectedIcon: '{{ $category->icon }}' }">
                    <label class="block mb-3 text-lg font-medium text-gray-900">
                        Choose an Icon
                    </label>
                    ...
                    <div class="flex flex-wrap gap-4">
                        ...
                        @foreach ($icons as $icon)
                            <button type="button" @click="selectedIcon = '{{ $icon }}'"
                                ...
                                <span class="material-symbols-outlined text-3xl text-[#954C2E]">
                                    {{ $icon }}
                                </span>
                            </button>
                        @endforeach
