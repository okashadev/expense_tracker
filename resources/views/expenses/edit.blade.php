<x-app-layout>
    <div class="px-4 py-2 w-full">
        ...
        <div class="w-full bg-white p-10 rounded-lg shadow-lg">
            <form action="{{ route('expenses.update', $expance->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="title" id="title"
                            ...
                            placeholder="Enter Expense Title" value="{{ $expance->title }}" required>
                    </div>
                    <div class="w-full">
                        <label for="amount" class="block mb-2 text-lg font-medium text-gray-900">Amount</label>
                        <input type="number" name="amount" id="amount"
                            class="bg-gray-50 border border-gray-300 text-black text-sm rounded-lg focus:ring-[#954C2E] focus:border-[#954C2E] block w-full p-2.5"
                            placeholder="$2999" value="{{ $expance->amount }}" required>
                    </div>
                    <div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea id="description" rows="4" name="description"
                            ...
                            placeholder="Your description here" >{{ $expance->description }}</textarea>
                    </div>
                </div>
